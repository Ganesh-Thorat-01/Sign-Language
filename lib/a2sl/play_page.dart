...
class PlayPage extends StatefulWidget {
  PlayPage({Key key, @required this.clips, @required this.sentence}) : super(key: key);
...
class _PlayPageState extends State<PlayPage> {
  ...
  set _isPlaying(bool value) {
    ...
    if (value) {
      _timerVisibleControl = Timer(Duration(seconds: 0), () {
        if (_disposed) return;
        setState(() {
          _controlAlpha = 0.0;
        });
      ...
      _timerVisibleControl = Timer(Duration(milliseconds: 0), () {
        if (_disposed) return;
        setState(() {
          _controlAlpha = 1.0;
        });
  ...
  void _initializeAndPlay(int index) async {
    ...
    final controller = clip["parent"].startsWith("http")
        ...
        : VideoPlayerController.asset("${clip['fileName']}");
        
    print("${clip['parent']}/${clip['fileName']}");
  ...
  Widget _buildCard(int index) {
    ...
    if (clip["runningTime"] > 60) {
      runtime = "${clip["runningTime"] ~/ 60}s ${clip["runningTime"] % 60}s";
    } else {
          ...
          children: <Widget>[
            Padding(
              ...
              child: clip["parent"].startsWith("http")
                  ...
                  : Image.asset("assets/Thumbnail.png", width: 70, height: 50, fit: BoxFit.fill),
            ),
            Expanded(
              child: Column(
